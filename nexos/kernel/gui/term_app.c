...
/* ── Simple built-in command dispatcher ─────────────────────────────────── */
...
static void term_paint(window_t *win) {
    ...
    fb_fill_rect(bx, by, win->w, win->h - WM_TITLEBAR_H, t->bg);

    for (int r = 0; r < TERM_ROWS; r++) {
        if (!t->buf[r][0] && r > t->row) break;
        font_puts(bx + TERM_PAD,
                  by + TERM_PAD + r * 16,
                  t->buf[r], t->fg, t->bg);
    }
    /* blinking cursor */
    ...
    if ((tick / 500) % 2 == 0) {
        int cx = bx + TERM_PAD + t->col * 8;
        int cy = by + TERM_PAD + t->row * 16;
        fb_fill_rect(cx, cy, 8, 16, COL_BLUE);
    }
}
...
term_app_t *term_create(int x, int y) {
    window_t *win = wm_new(x, y,
                           TERM_COLS * 8 + TERM_PAD * 2,
                           TERM_ROWS * 16 + WM_TITLEBAR_H + TERM_PAD * 2,
                           "Terminal");
    if (!win) return NULL;
    ...
    term_puts(t, "Shortcuts: Ctrl+T=Terminal  Ctrl+F=Files  Right-click=Menu\n");
    term_prompt(t);
